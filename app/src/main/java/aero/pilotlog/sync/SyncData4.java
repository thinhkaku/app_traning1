...
public class SyncData4 {

   */
/* private Context mContext;
    private SyncV4Fragment mFragment;
    private AutoPilot mAutoPilot;
    private VTDNav mVTDNavCurrency;
    private VTDNav mNavigation;
    public boolean isCalcel = false;
    public static int CURRENT_XML_VERSION = 9;
    private int version = 0;
    private int mIndex = 0;
    private ArrayList<String> downloadPictureList = new ArrayList<>();
    private boolean isExportCalendar;

    public SyncData4(Context pContext, SyncV4Fragment pFragment) {
        mContext = pContext;
    ...
    public boolean syncFromPC(SyncV4Fragment.CurrentFileName pCurrentFileName) {
        mIndex++;
        InputStream is;
        try {
            mFragment.sendStatusMessage(SyncStatusV4.DOWNLOAD_FILE, pCurrentFileName);
            is = new ByteArrayInputStream(DownloadAsync.downloadString(SyncConst.OUTBOX_V4 + pCurrentFileName.fileName).getBytes("UTF-8"));
            //PL-254
           *//*
*/
/* byte[] xmlFileByte = Utils.getBytes(is, pCurrentFileName.fileName, mContext);
            if(xmlFileByte.length==0){
                mFragment.sendStatusMessage(SyncStatusV4.ERROR_FILE_SIZE_ZERO, pCurrentFileName);
                return true;
            }*//*
*/
/*
            ...
            try {
                ...
                mFragment.sendStatusMessage(SyncStatusV4.ERROR_FILE_DAMAGED, pCurrentFileName);
                return true;
            }
            //End PL-507
            ...
            LogUtils.e(xmlContent);
            if (!xmlContent.contains("dataroot")) {
                mFragment.sendStatusMessage(SyncStatusV4.ERROR_SYNC_PC);
                return false;
            }

            //region: read XML tag
            mFragment.sendStatusMessage(SyncStatusV4.PROCESS_FILE, pCurrentFileName);
            if (!readTagVersion()) {
                return false;
            }
...
*/
/*
//    public boolean syncFromPC(String xmlContent) {
//        try {
//            VTDGen vg = new VTDGen();
    ...
    private boolean readTagVersion() {
        try {
            ...
            while (mAutoPilot.evalXPath() != -1) {
                LogUtils.e("/version ==> " + mNavigation.toNormalizedString(mNavigation.getText()));
                version = Integer.parseInt(mNavigation.toNormalizedString(mNavigation.getText()));
                if (version > CURRENT_XML_VERSION) {
                    mFragment.sendStatusMessage(SyncStatusV4.NEED_UPDATE);
                    return false;
                } else if (version < CURRENT_XML_VERSION - 1) {
                    mFragment.iStatus = SyncStatusV4.XML_TOO_OLD;
                    mFragment.sendStatusMessage(SyncStatusV4.XML_TOO_OLD);
                    return false;
                }
            }
        } catch (Exception e) {
...
/*
    private void readTagDelete() {
        ...
        try {
            ...
            if (mAutoPilot.evalXPath() != -1) {
                LogUtils.e(mNavigation.toString(mNavigation.getCurrentIndex()) + " ==> " + mNavigation.toNormalizedString(mNavigation.getText()));

                AutoPilot ap1 = new AutoPilot(mNavigation);
                ap1.selectXPath(".*//*
*/
/*");
//                mFragment.sendStatusMessage(SyncStatusV4.CLEANING_RECORD);//Remove as requite of customer
                ...
                if (replaceBy != null && (replaceBy.equalsIgnoreCase(MCCPilotLogConst.STRING_EMPTY) || replaceBy.equalsIgnoreCase(MCCPilotLogConst.NULL))) {
    ...
    private void readTagAircraft() {
        ...
        try {
            ...
            if (mAutoPilot.evalXPath() != -1 && mNavigation.getText() == -1) {
                mFragment.sendStatusMessage(SyncStatusV4.PROCESSING_AIRCRAFTS);
                AutoPilot ap1 = new AutoPilot(mNavigation);
...
/*");
    ...
    private void readTagAirfield() {
        ...
        try {
            ...
            if (mAutoPilot.evalXPath() != -1 && mNavigation.getText() == -1) {
                mFragment.sendStatusMessage(SyncStatusV4.PROCESSING_AIRFIELDS);
                LogUtils.e(mNavigation.toString(mNavigation.getCurrentIndex()) + " ==> ");
                AutoPilot ap = new AutoPilot(mNavigation);
                    ...
                    while (ap1.evalXPath() != -1) {
                        ...
                        switch (mNavigation.toString(mNavigation.getCurrentIndex())) {
                            case "AFCode":
                                ...
                                break;
                            case "AFCountry":
                                mAirfield.setAFCountry(Integer.parseInt(value.equalsIgnoreCase(MCCPilotLogConst.NULL) ? "0" : value));
                                break;
                            case "AFIATA":
                                mAirfield.setAFIATA(value.equalsIgnoreCase(MCCPilotLogConst.NULL) ? MCCPilotLogConst.STRING_EMPTY : value);
                                break;
...
/*
                ...
                mFragment.sendStatusMessage(SyncStatusV4.PROCESSING_CURRENCIES);
                *//*
*/
/**
...
/*
                while (ap.evalXPath() != -1) {
                    if (isCalcel)
                        break;
                    LogUtils.e(pVTDNav.toString(pVTDNav.getCurrentIndex()) + " ==> " *//*
*/
/*pVTDNav.toNormalizedString(pVTDNav.getText())*//*
*/
/*);
...
/*");
                    ...
                    while (ap1.evalXPath() != -1) {
                        if (isCalcel)
                            break;
                        LogUtils.e(pVTDNav.toString(pVTDNav.getCurrentIndex()) + " =====> " + pVTDNav.toNormalizedString(pVTDNav.getText()));
...
/*
                ...
                mFragment.sendStatusMessage(SyncStatusV4.PROCESSING_PILOT);
                *//*
*/
/**
                 * Create 2 loops, outer loops all the currency parent tag, inside loops all currency attributes likes expdate, text, ...
...
/*
                while (ap.evalXPath() != -1) {
                    ...
                    LogUtils.e(mNavigation.toString(mNavigation.getCurrentIndex()) + " ==> " *//*
*/
/*mNavigation.toNormalizedString(mNavigation.getText())*//*
*/
/*);
                    AutoPilot ap1 = new AutoPilot(mNavigation);
                    ap1.selectXPath(".*//*
...
/*");
                ...
                if (!isFirstFlight) {
                    isFirstFlight = true;
                    mFragment.sendStatusMessage(SyncStatusV4.PROCESSING_FLIGHTS);
                }
                while (ap.evalXPath() != -1) {
                    if (isCalcel)
                        break;
                    ...
                    switch (mNavigation.toString(mNavigation.getCurrentIndex())) {
                        ...
                        case "userN3":
                            ...
                            break;
                        case "minInstr":
                            mFlight.setMinInstr(Integer.parseInt(mNavigation.toNormalizedString(mNavigation.getText())));
                            break;
                        case "minNight":
                            mFlight.setMinNight(Integer.parseInt(mNavigation.toNormalizedString(mNavigation.getText())));
                            break;
                        case "minPIC":
                            mFlight.setMinPIC(Integer.parseInt(mNavigation.toNormalizedString(mNavigation.getText())));
                            break;
                        case "minPICus":
                            mFlight.setMinPICus(Integer.parseInt(mNavigation.toNormalizedString(mNavigation.getText())));
                            break;
                        case "minSFR":
                            mFlight.setMinSFR(Integer.parseInt(mNavigation.toNormalizedString(mNavigation.getText())));
                            break;
                        case "minTotal":
                            mFlight.setMinTotal(Integer.parseInt(mNavigation.toNormalizedString(mNavigation.getText())));
                            break;
                        case "minXC":
                            mFlight.setMinXC(Integer.parseInt(mNavigation.toNormalizedString(mNavigation.getText())));
                            break;
                        case "P1Code":
                        ...
                        case "TypeOfLaunch":
                            ...
                            break;
                        case "DeIce":
                            mFlight.setDeice(Integer.parseInt(mNavigation.toNormalizedString(mNavigation.getText())));
                            break;
                        case "Delay":
                ...
                if (!MCCPilotLogConst.ZERO_STRING.equals(mFlight.getFlightCode()) && acceptedSaveFlight) {
                    SyncViewModel.addFlightFromPc(mContext, mFlight);
...
/*");
                mTotals = new Totals();
                mFragment.sendStatusMessage(SyncStatusV4.UPDATING_TOTALS);
...
*/
/*");
                mFragment.sendStatusMessage(SyncStatusV4.UPDATING_GRAND_TOTALS);
    ...
    private void readTagLogbook() {
        ...
        try {
            ...
            if (mNavigation.getText() == -1) {
                mFragment.sendStatusMessage(SyncStatusV4.PROCESSING_LOGBOOK_FLIGHTS);
            }
            while (mAutoPilot.evalXPath() != -1 && mNavigation.getText() == -1) {
                if (isCalcel)
                    break;
...
//                for(String s : logbookFlightList){
//                    LogUtils.e("logbook record =>>>>>>  " + s);
//                }
//                SyncViewModel.addLogbookFromPc(mContext, logbookFlightList, version);
            }
            //TuanPV add new
            if (!logbookFlightList.isEmpty()) {
                LogUtils.e("size logbookFlightList v4.0: " + logbookFlightList.size());
                SyncViewModel.addLogbookFromPc(mContext, logbookFlightList, version);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void readTagPicture() {
        try {
            mAutoPilot.selectXPath("//IM");
            while (mAutoPilot.evalXPath() != -1) {
                if (isCalcel)
                    break;
                mFragment.intPicture++;
                downloadPictureList.add(mNavigation.toNormalizedString(mNavigation.getText()));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        try {
            ...
            while (mAutoPilot.evalXPath() != -1) {
                ...
                if (!isFirstCalendar) {
                    ...
                    mFragment.sendStatusMessage(SyncStatusV4.PROCESSING_ICAL);
                }
                AutoPilot ap1 = new AutoPilot(mNavigation);
                ap1.selectXPath(".*//*
*/
...
//                if (TextUtils.isEmpty(DatabaseManager.getInstance(mContext).getSetting(SettingsConst.DEFAULT_CALENDAR).getData().trim())){
//                    mFragment.sendStatusMessage(SyncStatusV4.ERROR_CALENDAR_NO_NAME);
//                }else {
...
//                }
    ...
    public boolean createUploadFile(Context pContext, String fileName, int logNumber, boolean bNotComplete, List<String> pPictureList) {
        ...
        if (!pPictureList.isEmpty()) {
            xmlFile.append("<IM>\n");
            for (String imageFile : pPictureList) {
                xmlFile.append("<img>").append(imageFile).append("</img>").append("\n");
            }
            xmlFile.append("</IM>\n");
        }
        ...
        if (!flightsToSync.isEmpty() || !pilotsToSync.isEmpty() || !airfieldsToSync.isEmpty() || !pPictureList.isEmpty()) {
            hasData = true;
                ...
                final int numberOfLogFile = StorageUtils.getIntFromSharedPref(pContext, StateKey.LAST_SYNC_LOG_FILE, 0);
                {
                    if (numberOfLogFile >= 20) {
                        ...
                        if (f.exists() && f.isDirectory()) {
                            while (f.listFiles().length > 0 && f.listFiles().length < 20) {
    ...
    public ArrayList<String> getDownloadPictureList() {
        return downloadPictureList;
    }

    public ArrayList<File> getUploadPictureList() {
        ArrayList<File> uploadPictureList = new ArrayList<>();
        File rootFolder = new File(StorageUtils.getStorageRootFolder(mContext));

        //get all pilot picture to sync
        File pilotPictureFolder = new File(rootFolder, MCCPilotLogConst.PICTURE_FOLDER);
        File[] pictures = pilotPictureFolder.listFiles(new FilenameFilter() {
            @Override
            public boolean accept(File dir, String filename) {
                return filename.contains("zip") && filename.contains("pict");
            }
        });
        if (pictures != null && pictures.length > 0) {
            uploadPictureList.addAll(Arrays.asList(pictures));
        }

        //get all sign picture to sync
        File signFolder = new File(rootFolder, MCCPilotLogConst.SIGN_FOLDER);
        File[] signs = signFolder.listFiles(new FilenameFilter() {
            @Override
            public boolean accept(File dir, String filename) {
                return filename.contains("zip") && filename.contains("sign");
            }
        });
        if (signs != null && signs.length > 0) {
            uploadPictureList.addAll(Arrays.asList(signs));
        }
